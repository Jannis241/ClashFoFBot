...
impl ScreenshotApp {
    ...
    fn show_selectable_models(&mut self, ui: &mut egui::Ui) {
        ...
        models.sort_by(|a, b| {
            let rating_res_a = image_data_wrapper::get_rating(&b.clone());
    ...
    fn save_labeld_rects(&mut self, final_size: egui::Vec2) {
        if let Some(image_path) = self.labeling_que.clone().last() {
            ...
            let yaml_content = match fs::read_to_string(yaml_path) {
                Ok(content) => content,
                Err(_) => {
                    self.create_error("Konnte data.yaml nicht lesen.", MessageType::Error);
                    return;
                }
            };
            ...
            let mut data: DataYaml = match serde_yaml::from_str(&yaml_content) {
                Ok(parsed) => parsed,
                Err(_) => {
                    self.create_error("Konnte data.yaml nicht parsen.", MessageType::Error);
                    return;
                }
            };
            ...
            {
                self.create_error("Bild konnte nicht kopiert werden.", MessageType::Error);
                return;
            }
            ...
            if fs::create_dir_all(label_target).is_err() {
                self.create_error(
                    "Label-Ordner konnte nicht erstellt werden.",
                    MessageType::Error,
                );
                return;
            }
            ...
            let mut label_file = match fs::File::create(&label_path) {
                ...
                Err(_) => {
                    self.create_error(
                        "Konnte .txt-Datei für Labels nicht schreiben.",
                        MessageType::Error,
                    );
                    return;
                }
            ...
            for lr in &self.labeled_rects {
                ...
                .is_err()
                {
                    self.create_error("Fehler beim Schreiben der Label-Datei.", MessageType::Error);
                    return;
                }
            }

            if yaml_updated {
                let new_yaml = match serde_yaml::to_string(&data) {
                    Ok(s) => s,
                    Err(_) => {
                        self.create_error("Fehler beim YAML-Serialisieren.", MessageType::Error);
                        return;
                    }
                };
                if fs::write(yaml_path, new_yaml).is_err() {
                    self.create_error("Fehler beim Schreiben von data.yaml.", MessageType::Error);
                    return;
                }
                self.create_error(
                    "Neue Klassen wurden zu data.yaml hinzugefügt.",
                    MessageType::Success,
                );
            }
